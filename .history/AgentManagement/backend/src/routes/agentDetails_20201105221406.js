...
router.get("/getProfile", async function (req, res) {
    ...
    agentDetails.findd({ $and: [{"agentID": agentID},{ "organisationID": organisationID}] }).then(agent => {
        console.log(agent)
        if (!agent) {
            res.status(500).json({ responseMessage: 'Cases not found' });
        } else {
...
router.post('/updateProfile', function (req, res) {
    ...
    agentDetails.findOneAndUpdate(({ $and: [{"AgentID": agentID},{ "organisationID": organisationID}]},{ $addToSet:{"phoneNumber":phoneNumber, "password":password}}).then(user => {
        console.log(user)
        if (!user) {
