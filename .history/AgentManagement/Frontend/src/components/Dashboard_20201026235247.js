...
import config from '../config/settings'
 ...
 class Dashboard extends Component {
      ...
      params: data,
       ...
       .then((response) => {
         ...
         if(response.status === 200){
          let receivedCases = response.cases;
          if(receivedCases){
            this.setState({ allCases: receivedCases });
          }else{
   ...
   render() {
     ...
     var casedetails;
     if (this.state.allCases !== null && this.state.allCases !== undefined && this.state.allCases !=={}) {
       casedetails = this.state.allCases.map((cas) => {
         return (
